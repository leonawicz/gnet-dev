# @knitr setup_cities_seq_combine
# Combine cities sequence frame batches
...
ws.files <- list.files("intermediary", pattern=paste0(name, "_breakDL_ws.*.RData"), full=TRUE)
...
system.time( groups.list <- lapply(p.list.final, function(x) unique(x$grp[!is.na(x$lon)])) )
n.frames <- sapply(groups.list, length)
print(sum(n.frames)) # total pathway lines: number of concurrent pathways * (this sum OR the fixed 4800 below)
...
n.frames <- 5400 # v3 # 4800 for v2
n.intro <- 1800
size.intro <- 1000
n.outro <- 2400 # v3 # 1800 for v2
...
dateline.break <- TRUE
file.out <- if(dateline.break) paste0(name, "_breakDL_final.RData") else paste0(name, "_continuous_final.RData")
