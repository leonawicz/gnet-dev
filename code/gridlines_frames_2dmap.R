...
load("gridlines_sequence2_final.RData")
#load("points.RData")
...
# @knitr 2dmap_frames
# Generate frames
framesetID <- "tpbg_gridlinesseq2_OrgWht"
...
start <- Sys.time()
mclapply(frames, save2Dplot, outDir=outDir, w=w, h=h, pts=NULL, bg="transparent", increase.alpha=30, check.dl.brk=TRUE, txt=NULL, mc.cores=n.cores)
end <- Sys.time()
difftime(end, start)
